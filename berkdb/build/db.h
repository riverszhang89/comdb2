...
#define	DB_LOGFILEID_INVALID	-1
	...
	/* 1 if using thread-local cursor queues. Only indexes will have the flag
	   lit. We don't create thread-local cursor queues for data, blob or
	   temptables, because there's already little (data and blob are striped)
	   or zero (temptables are single-threaded) contention on these resources. */
	int use_tlcq;
