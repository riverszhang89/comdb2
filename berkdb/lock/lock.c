...
expired:			obj_ndx = sh_obj->index;
    ...
    if (master_locker->dd_in_wlocks) {
        ...
        if (master_locker->dd_in_wlocks) {
            ...
            master_locker->dd_in_wlocks = 0;
        }
        unlock_detector(region);
    }

    master_locker->wstatus = 0;
