...
static int __db_get_mp_multiple __P((DB *, double *));
static int __db_set_mp_multiple __P((DB *, double));
	...
	u_int32_t flags;
{
	...
	/* Give index files a 50% priority advantage in the buffer pool. */
	if (idxpri && (ret = dbp->mpf->set_priority(dbp->mpf,
		    DB_PRIORITY_INDEX)) != 0)
		goto err;
	...
	u_int32_t flags;
{
	...
	dbp->paired_cursor_from_txn = __db_cursor_pp_paired_from_txn;
	dbp->paired_cursor_from_lid = __db_cursor_pp_paired_from_lid;
	dbp->cursor_nocount = __db_cursor_pp_nocount;
	dbp->cursor_ser = __db_cursor_ser_pp;
	...
	dbp->get_mp_multiple = __db_get_mp_multiple;
	dbp->set_mp_multiple = __db_set_mp_multiple;
...
/*
 * __db_get_cachesize --
 *	Get multiplier for number of hash buckets. More buckets are better
 *      for smaller pages, while fewer buckets are necessary for larger
 *      pages. Additionally, having more buckets reduces collisions, 
 *      while fewer buckets results in a more optimal LRU.
 */
static int
__db_get_mp_multiple(dbp, multiple)
	DB *dbp;
	double *multiple;
{
	DB_ILLEGAL_IN_ENV(dbp, "DB->get_mp_multiple");

	return (__memp_get_mp_multiple(dbp->dbenv, multiple));
}

/*
 * __db_set_cachesize --
 *	Set multiplier for number of hash buckets. More buckets are better
 *      for smaller pages, while fewer buckets are necessary for larger
 *      pages. Additionally, having more buckets reduces collisions, 
 *      while fewer buckets results in a more optimal LRU.
 */
static int
__db_set_mp_multiple(dbp, multiple)
	DB *dbp;
	double multiple;
{
	DB_ILLEGAL_IN_ENV(dbp, "DB->set_mp_multiple");
	DB_ILLEGAL_AFTER_OPEN(dbp, "DB->set_mp_multiple");

	return (__memp_set_mp_multiple(dbp->dbenv, multiple));
}
