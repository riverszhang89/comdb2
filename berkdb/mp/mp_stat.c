	...
	u_int32_t flags;
{
	...
	if ((ret = __db_fchk(dbenv,
	    "DB_ENV->memp_stat", flags, DB_STAT_CLEAR | DB_STAT_MINIMAL)) != 0)
		return (ret);
	...
	if (gspp != NULL) {
		*gspp = NULL;
		...
		for (i = 0; i < mp->nreg; ++i) {
			...
			sp->st_cache_ihit += c_mp->stat.st_cache_ihit;
			sp->st_cache_imiss += c_mp->stat.st_cache_imiss;
			sp->st_cache_lhit += c_mp->stat.st_cache_lhit;
			sp->st_cache_lmiss += c_mp->stat.st_cache_lmiss;
			...
			sp->st_rw_merges += c_mp->stat.st_rw_merges;
			if (LF_ISSET(DB_STAT_MINIMAL))
				continue;
			sp->st_ro_evict += c_mp->stat.st_ro_evict;
			sp->st_rw_evict += c_mp->stat.st_rw_evict;
			sp->st_ro_levict += c_mp->stat.st_ro_levict;
			sp->st_rw_levict += c_mp->stat.st_rw_levict;
			sp->st_pf_evict += c_mp->stat.st_pf_evict;
			sp->st_rw_evict_skip += c_mp->stat.st_rw_evict_skip;
			...
			if (sp->st_alloc_max_pages <
				...
				sp->st_alloc_max_pages =
				    c_mp->stat.st_alloc_max_pages;
			sp->st_ckp_pages_sync += c_mp->stat.st_ckp_pages_sync;
			sp->st_ckp_pages_skip += c_mp->stat.st_ckp_pages_skip;
	...
	FILE *fp;
{
	...
	for (cnt = 0, mfp = SH_TAILQ_FIRST(&mp->mpfq, __mpoolfile);
		...
		(void)logmsgf(LOGMSG_USER, fp, "File #%d: %s: pagesize %lu\n", cnt + 1,
		    __memp_fns(dbmp, mfp), (u_long)mfp->stat.st_pagesize);
		(void)logmsgf(LOGMSG_USER, fp, "\t type %ld; ref %lu; blocks %lu; last %lu;",
		...
		for (i = 0; i < DB_FILE_ID_LEN; ++i, ++p) {
			(void)logmsgf(LOGMSG_USER, fp, "%x", (u_int)*p);
	...
	for (dbmfp = TAILQ_FIRST(&dbmp->dbmfq);
		...
		(void)logmsgf(LOGMSG_USER, fp, "File #%d: %s: per-process, %s\n",
	...
	if (LF_ISSET(MPOOL_DUMP_HASH)) {
		...
		for (hp = R_ADDR(reginfo, c_mp->htab),
			...
			if ((bhp =
				...
				(void)logmsgf(LOGMSG_USER, fp, "%lu (%u):\n",
	...
	FILE *fp;
{
	...
	if (fmap[i] == INVALID_ROFF)
		...
		(void)logmsgf(LOGMSG_USER, fp, "\t%5lu,   #%d,  %2lu, %8lu [%lu,%lu] %lu",
		    (u_long)bhp->pgno, i + 1,
		    (u_long)bhp->ref, (u_long)R_OFFSET(dbmp->reginfo, bhp),
		    (u_long)LSN(bhp->buf).file, (u_long)LSN(bhp->buf).offset,
		    (u_long)bhp->priority);
