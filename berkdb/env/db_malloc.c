...
int
__db_shalloc_malloc_offset(void *p, size_t len, size_t align, size_t offset, void *retp)
{
	int bucket;
	void *rp;
	HEAP *h = *(HEAP **)p;
	...
	(void)comdb2_posix_memalign_offset(h->msp, &rp, align, len, offset);
	if (rp == NULL)
		return ENOMEM;
...
{
	return __db_shalloc_malloc_offset(p, len, align, 0, retp);
}
