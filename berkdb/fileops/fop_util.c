	...
	u_int32_t flags, *retidp;
{
	...
	*retidp = TXN_INVALID;
	...
	/* open with directio if they specified it in the env! */
...
reopen:		if ((ret = __os_open(dbenv, real_name, oflags, 0, &fhp)) != 0)
		...
		if ((ret = __db_meta_setup(dbenv,
			...
			/* Check for metapage in recovery_page cache:
			 * always idx 0. */
			if (dbenv->mp_recovery_pages > 0) {
				/* Allocate recovery page filename. */
				if ((ret = __os_calloc(dbenv, 1,
					    strlen(real_name) +
					    strlen(recp_ext) + 1,
					    &recp_name)) != 0) {
					__db_err(dbenv,
					    "Error allocating %s file: %s",
					    recp_ext,
					    db_strerror(ret));
					goto err;
				}

				/* Form recovery_page filename. */
				sprintf(recp_name, "%s%s", real_name, recp_ext);

				/* Open the recovery_page file. */
				if ((ret = __os_open(dbenv, recp_name,
					    oflags, 0, &recp)) != 0) {
					__db_err(dbenv,
				...
				/* Read in the meta-data page. */
				if ((ret =
					__fop_read_meta(dbenv, recp_name, mbuf,
					    sizeof(mbuf), recp,
					    LF_ISSET(DB_FCNTL_LOCKING)
					    && txn == NULL ? 1 : 0, &len)) != 0) {
					__db_err(dbenv,
		...
		if ((ret = __fop_lock_handle(dbenv,
			...
			/* open with directio if they specified it in the env! */
			oflags = 0;
			if (F_ISSET(dbenv, DB_DIRECT_DB))
				oflags |= DB_OSO_DIRECT;

			if ((ret =
				__os_open(dbenv, real_name, oflags, 0,
				    &fhp)) != 0)
				goto err;
		} else
			goto err;
