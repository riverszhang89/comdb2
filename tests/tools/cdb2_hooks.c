...
int main(int argc, char **argv)
{
    ...
    rc = TEST_init_once_registration(argv[1], tier);
    if (rc != 0)
        return rc;
    ...
    rc = TEST_simple_register_unregister(argv[1], tier);
    if (rc != 0)
        return rc;

    puts("====== EVENT WITH ADDITIONAL INFORMATION ======");
    rc = TEST_arg_events(argv[1], tier);
    if (rc != 0)
        return rc;

    puts("====== EVENT THAT INTERCEPTS AND OVERWRITES THE RETURN VALUE ======");
    rc = TEST_modify_rc_event(argv[1], tier);
    if (rc != 0)
        return rc;
    return 0;
}
