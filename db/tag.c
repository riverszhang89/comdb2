...
int upd_master_columns(struct ireq *iq, void *intrans, void *record, size_t reclen)
{
    ...
    int rc = 0, bdberr = 0;
    int64_t val;
    struct schema *schema = get_ondisk_schema(iq->usedb, -1);
...
int describe_update_columns(const struct ireq *iq, const struct schema *chk, int *updCols)
{
    ...
    ondisk = get_ondisk_schema(iq->usedb, -1);
    if (ondisk == NULL) {
        return -1;
    }
                           ...
                           struct convert_failure *fail_reason)
{
    return _stag_to_stag_buf_flags_blobs(find_tag_schema(table, fromtag), find_tag_schema(table, totag), inbuf, outbuf,
                                         flags, fail_reason, NULL /*inblobs*/, NULL /*outblobs*/, 0 /*maxblobs*/, NULL);
}
...
int create_key_from_ondisk_sch_blobs(const struct dbtable *db, int ixnum, char **tail, int *taillen, char *mangled_key,
                                     const char *inbuf, int inbuflen, char *outbuf, struct convert_failure *reason,
                                     blob_buffer_t *inblobs, int maxblobs, const char *tzname)
{
    ...
    rc = _stag_to_stag_buf_flags_blobs(fromsch, tosch, inbuf, outbuf, 0 /*flags*/, reason, inblobs, NULL /*outblobs*/,
                                       maxblobs, tzname);
    if (rc)
        return rc;
...
int create_key_from_ondisk(const struct dbtable *db, int ixnum, char **tail, int *taillen, char *mangled_key,
                           const char *inbuf, int inbuflen, char *outbuf, const char *tzname)
{
    return create_key_from_ondisk_sch_blobs(db, ixnum, tail, taillen, mangled_key, inbuf, inbuflen, outbuf, NULL,
                                            NULL /*inblobs*/, 0 /*maxblobs*/, tzname);
}

int create_key_from_ondisk_blobs(const struct dbtable *db, int ixnum, char **tail, int *taillen, char *mangled_key,
                                 const char *inbuf, int inbuflen, char *outbuf, blob_buffer_t *inblobs, int maxblobs)
{
    return create_key_from_ondisk_sch_blobs(db, ixnum, tail, taillen, mangled_key, inbuf, inbuflen, outbuf, NULL,
                                            inblobs, maxblobs, NULL);
}
...
{
    return (ix == -1) ? usedb->schema : usedb->ixschema[ix];
}
