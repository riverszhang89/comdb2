...
static int start_schema_change_tran_repartition(const char *tblname, timepart_sc_arg_t *arg)
{
    ...
    if (arg->indx == 0) {
        ...
        //newsc->unpublish = partition_unpublish;
    }
#if 0
    newsc->iq = sc->iq;
    newsc->tran = sc->tran;
#endif
    newsc->kind = sc->kind;
    if (arg->indx == 0) /* treat 1st shard as a regular rebuild */
        newsc->partition.type = PARTITION_NONE;
    else if (arg->indx < sc->partition.u.tpt.retention) /* merge + fastinit */
    ...
    printf("%s: !!!!!!!!!!!!!!!!! newsc kind is %d partition type %d\n", __func__, newsc->kind, newsc->partition.type);
    rc = start_schema_change_tran(iq, NULL);
...
static int _process_partition_alter_and_drop(struct ireq *iq)
{
    ...
    if (sc->kind == SC_ALTERTABLE) { /* this is a re-partition */
        rc = _process_partitioned_table_repartition(iq);
        if (rc != 0)
            return rc;
        return rc;
    }

    assert(sc->kind == SC_DROPTABLE || sc->kind == SC_ALTERTABLE);
