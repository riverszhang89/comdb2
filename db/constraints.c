    ...
    for (i = 0; i < iq->usedb->n_rev_constraints; i++) {
        ...
        for (j = 0; j < cnstrt->nrules; j++) {
            ...
            if (iq->idxDelete)
                ...
                rc =
                    create_key_from_ondisk(iq->usedb, ixnum, NULL, NULL, NULL, rec_dta, 0 /* not needed */, lkey, NULL);
            if (rc == -1) {
                if (iq->debug)
                    reqprintf(iq,
            ...
            if (newrec_dta != NULL) {
                if (iq->idxInsert)
                    ...
                    rc = create_key_from_ondisk(iq->usedb, ixnum, NULL, NULL, NULL, newrec_dta, 0 /* not needed */,
                                                nkey, NULL);
                if (rc == -1) {
                    if (iq->debug)
                        reqprintf(
        ...
        case BLOCK_UPVRRN: {
            ...
            for (cidx = 0; cidx < nct; cidx++) {
                ...
                if (iq->idxInsert) {
                    ...
                    rc = create_key_from_ondisk(iq->usedb, lixnum, NULL, NULL, NULL, od_dta, 0 /* not needed */, lkey,
                                                NULL);
