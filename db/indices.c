                       ...
                       int *ixfailnum, int *retrc, void *od_dta, size_t od_len, unsigned long long ins_keys)
{
    ...
    if (iq->idxInsert)
        ...
        rc = create_key_from_ondisk_sch_blobs(iq->usedb, ixnum, &od_dta_tail, &od_tail_len, mangled_key, od_dta, od_len,
                                              key, NULL, blobs, maxblobs, iq->tzname);
    }
    if (rc == -1) {
        if (iq->debug)
                       ...
                       int flags, bool reorder)
{
    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (iq->idxInsert)
            ...
            rc = create_key_from_ondisk_sch_blobs(iq->usedb, ixnum, &od_dta_tail, &od_tail_len, mangled_key, od_dta,
                                                  od_len, key, NULL, blobs, maxblobs, iq->tzname);
        }
        if (rc == -1) {
            if (iq->debug)
                reqprintf(iq, "CAN'T FORM INDEX %d", ixnum);
                       ...
                       unsigned long long vgenid, int *deferredAdd)
{
    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (iq->idxDelete) {
            ...
            rc = create_key_from_ondisk_blobs(iq->usedb, ixnum, &od_olddta_tail, &od_oldtail_len, mangled_oldkey,
                                              old_dta, od_len, oldkey, del_idx_blobs, del_idx_blobs ? MAXBLOBS : 0);
        if (rc < 0) {
            if (iq->debug)
        ...
        if (iq->idxInsert) {
            ...
            rc = create_key_from_ondisk_blobs(iq->usedb, ixnum, &od_dta_tail, &od_tail_len, mangled_newkey, od_dta,
                                              od_len, newkey, add_idx_blobs, add_idx_blobs ? MAXBLOBS : 0);
        if (rc < 0) {
            if (iq->debug)
    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (iq->idxDelete)
            ...
            rc = stag_to_stag_buf_blobs(iq->usedb->tablename, ondisktag, od_dta,
                                        ixtagptr, key, NULL, del_idx_blobs,
                               ...
                               int verify)
{
    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (iq->idxInsert)
            ...
            rc = create_key_from_ondisk_blobs(iq->usedb, ixnum, &od_dta_tail, &od_tail_len, mangled_key,
                                              (char *)new_dta, nd_len, key, blobs, blobs ? MAXBLOBS : 0);
        }
    ...
    int deferredAdd)
{
    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (iq->idxDelete) {
            ...
            rc = create_key_from_ondisk_blobs(iq->usedb, ixnum, NULL, NULL, NULL,
                                              use_new_tag ? (char *)sc_old : (char *)old_dta, 0 /*not needed*/, key,
                                              del_idx_blobs, del_idx_blobs ? MAXBLOBS : 0);
        }
