                                         ...
                                         int *bdberr, unsigned long long seq)
{
    ...
    rc = osql_send_updcols(osql, savkey,
                                       ...
                                       char *record)
{
    ...
    for (i = 0; i < tbl->nblobs; i++) {
        ...
        rc = osql_send_qblob(osql, idx, seq,
                                       ...
                                       unsigned long long seq, int is_delete)
{
    ...
    for (i = 0; i < tbl->nix; i++) {
        ...
        rc = osql_send_index(osql, seq, is_delete,
...
static int process_local_shadtbl_sc(struct sqlclntstate *clnt, int *bdberr)
{
    ...
    while (rc == 0) {
        ...
        rc = osql_send_schemachange(osql, sc,
                                    NET_OSQL_SOCK_RPL);
        if (rc) {
            logmsg(LOGMSG_ERROR,
                   "%s: error writting record to master in offload mode!\n",
...
static int process_local_shadtbl_bpfunc(struct sqlclntstate *clnt, int *bdberr)
{
    ...
    while (rc == 0) {
        ...
        rc = osql_send_bpfunc(osql, func->arg,
                              NET_OSQL_SOCK_RPL);
        free_bpfunc(func);
