    ...
    if (inconsistent) {
        ...
        */
        ...
        if (!blob->notnull && b->blobptrs[cblob] != NULL) {
            ...
            /* There was a bug that when an inline blob record was updated to
               something that could completely fit in the inline portion,
               1) the off-page blob of the inline record wouldn't be removed;
               2) the blob length wouldn't be updated.
