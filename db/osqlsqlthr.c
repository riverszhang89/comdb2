...
retry:
    ...
    rc = osql_comm_send_socksqlreq(osql, clnt->sql,
                                   strlen(clnt->sql) + 1,
                                   clnt->tzname, type, flags);
...
static int osql_send_del_logic(struct BtCursor *pCur, struct sql_thread *thd)
{
    ...
    if (osql->is_reorder_on) {
        rc = osql_send_delrec(osql, pCur->genid,
                               ...
                               blob_buffer_t *blobs, int maxblobs, int flags)
{
    ...
    if (updCols) {
        rc = osql_send_updcols(osql, pCur->genid,
...
int osql_serial_send_readset(struct sqlclntstate *clnt, int nettype)
{
    ...
    rc = osql_send_serial(osql, arr_ptr,
                                     ...
                                     int nettype)
{
    ...
    do {
        rc = osql_send_usedb(osql, tablename,
                                   ...
                                   int nData, int nStat, int nettype)
{
    ...
    START_SOCKSQL;
    do {
        rc = osql_send_updstat(osql, pCur->genid,
                                  ...
                                  struct sqlclntstate *clnt, int nettype)
{
    ...
    for (i = 0; i < pCur->db->nix; i++) {
        ...
        if (gbl_partial_indexes && pCur->db->ix_partial &&
            ...
            continue;
        rc = osql_send_index(osql, pCur->genid, 0,
                                  ...
                                  struct sqlclntstate *clnt, int nettype)
{
    ...
    for (i = 0; i < pCur->db->nix; i++) {
        ...
        rc = osql_send_index(osql, pCur->genid,
...
int osql_query_dbglog(struct sql_thread *thd, int queryid)
{
    ...
    do {
        rc = osql_send_dbglog(osql, new_cookie,
                     ...
                     genid_t genid)
{
    ...
    if (rc != SQLITE_OK)
        return rc;
    return osql_send_dbq_consume(osql, genid,
                                 NET_OSQL_SOCK_RPL);
}
