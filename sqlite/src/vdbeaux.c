...
int sqlite3MemCompare(const Mem *pMem1, const Mem *pMem2, const CollSeq *pColl){
  ...
  */
  ...
  if(combined_flags&MEM_Datetime || combined_flags&MEM_Interval) {
    ...
    if( err ){
      /* If we have a VDBE, set its rc. Otherwise print a warning. */
      struct Vdbe *vdbe = NULL;
      if( pMem1->db!=NULL )
        vdbe = pMem1->db->pVdbe;
      else if( pMem2->db!=NULL )
        vdbe = pMem2->db->pVdbe;
      if( vdbe!=NULL ){
        vdbe->rc = SQLITE_CONV_ERROR;
      }else{
        logmsg(LOGMSG_ERROR, "%s: encountered a type error from compareDateTimeInterval()\n", __func__);
      }
    }
    return rc;
  }
