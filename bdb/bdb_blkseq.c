...
static DB *create_blkseq(bdb_state_type *bdb_state, int stripe)
{
    char fname[1024];
    DB *db;
    DB_ENV *env;
    ...
    snprintf(fname, sizeof(fname), "%s/_blkseq.%d.XXXXXX", bdb_state->tmpdir,
             stripe);
    fd = mkstemp(fname);
    if (fd == -1) {
...
int bdb_create_private_blkseq(bdb_state_type *bdb_state)
{
    ...
    for (int stripe = 0; stripe < nstripes; stripe++) {
        ...
        for (int i = 0; i < 2; i++) {
            bdb_state->blkseq[i][stripe] = create_blkseq(bdb_state, stripe);
...
int bdb_blkseq_clean(bdb_state_type *bdb_state, uint8_t stripe)
{
    ...
    newdb = create_blkseq(bdb_state, stripe);
    if (newdb == NULL) {
        rc = BDBERR_MISC;
        goto done;
    }
