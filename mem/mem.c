...
{
	return comdb2_posix_memalign_offset(cm, mem, alignment, size, 0);
}

int comdb2_posix_memalign_offset(comdb2ma cm, void **mem, size_t alignment, size_t size, size_t offset)
{
    void **out = NULL;
    ...
    if (size > COMDB2MA_MAX_MEM) {
        ...
        if (!COMDB2MA_FULL(cm))
			out = mspace_memalign_offset(cm->m, alignment, size + COMDB2MA_OVERHEAD(d), COMDB2MA_HEAD_SIZE + offset);
