...
static MUTEX_T cdb2_ssl_sess_lock = PTHREAD_MUTEX_INITIALIZER;
...
static MUTEX_T cdb2_sockpool_mutex = MUTEX_INITIALIZER;
static ONCE_T init_once = ONCE_INIT;
...
#include <fcntl.h>
static int lclconn(SOCKET s, const struct sockaddr *name, int namelen,
                   int timeoutms)
{
...
static SOCKET cdb2_do_tcpconnect(struct in_addr in, int port, int myport,
                              int timeoutms)
{
    SOCKET sockfd;
    int rc;
...
static SOCKET cdb2_tcpconnecth_to(const char *host, int port, int myport,
                                  int timeoutms)
{
    int rc;
    struct in_addr in;
                              ...
                              int *comdb2db_found)
{
    ...
    while (read_line((char *)&line, sizeof(line), fp, buf, &line_no) != -1) {
        ...
        if (tok == NULL)
            ...
            MUTEX_LOCK(&cdb2_sockpool_mutex);
...
static int try_ssl(cdb2_hndl_tp *hndl, SBUF2 *sb, int indx)
{
    ...
    if (cdb2_cache_ssl_sess) {
        if (hndl->sess_list == NULL) {
            ...
            rc = MUTEX_LOCK(&cdb2_ssl_sess_lock);
            if (rc != 0) {
                ...
                MUTEX_UNLOCK(&cdb2_ssl_sess_lock);
            }
        }
...
static SOCKET cdb2portmux_route(const char *remote_host, char *app, char *service,
                                const char *instance)
{
    char name[64];
    char res[32];
                          ...
                          int timeoutms, int indx)
{
    ...
    if (hndl->debug_trace) {
        fprintf(stderr, "td %u %s line %d newsql_connect\n",
                (uint32_t)SELF(), __func__, __LINE__);
    }
...
static int newsql_disconnect(cdb2_hndl_tp *hndl, SBUF2 *sb, int line)
{
    ...
    if ((hndl->firstresponse &&
        ...
        sbuf2free(sb);
        cdb2_socket_pool_donate_ext(hndl->newsql_typestr, sbuf2fileno(sb), 10,
...
retry_connect:
    ...
    if (hndl->sb == NULL) {
        ...
        memset(hndl->hosts_connected, 0, sizeof(hndl->hosts_connected));
    ...
    *len = hdr.length;
    ...
    if (hdr.type == RESPONSE_HEADER__SQL_RESPONSE_TRACE) {
        ...
        if (response->response_type == RESPONSE_TYPE__SP_TRACE) {
            ...
            query.spcmd = cmd;
            int dtalen = (int)cdb2__query__get_packed_size(&query);
            unsigned char *dtabuf = malloc(dtalen + 1);

            cdb2__query__pack(&query, dtabuf);

            struct newsqlheader dtahdr;

            dtahdr.type = ntohl(CDB2_REQUEST_TYPE__CDB2QUERY);
            dtahdr.compression = ntohl(0);
            dtahdr.length = ntohl(dtalen);

            sbuf2write((char *)&dtahdr, sizeof(dtahdr), hndl->sb);
            sbuf2write((char *)dtabuf, dtalen, hndl->sb);

            rc = sbuf2flush(hndl->sb);
            if (rc < 0)
                return -1;
            free(dtabuf);
        }
...
static int cdb2_effects_request(cdb2_hndl_tp *hndl)
{
    ...
    int len = (int)cdb2__query__get_packed_size(&query);
...
static int cdb2_send_query(cdb2_hndl_tp *hndl, SBUF2 *sb, char *dbname,
                           ...
                           int ntypes, int *types, int is_begin, int64_t skip_nrows,
                           int retries_done, int do_append, int fromline)
{
    ...
    if (hndl && hndl->debug_trace) {
        fprintf(stderr, "td %u %s sending '%s' to %s from-line %d retries is "
                        "%d do_append is %d\n",
                (uint32_t)SELF(), __func__, sql, host, fromline,
...
int cdb2_get_effects(cdb2_hndl_tp *hndl, cdb2_effects_tp *effects)
{
    ...
    if (log_calls) {
        fprintf(stderr, "%p> cdb_get_effects(%p) = %d",
                (void *)(intptr_t)SELF(), hndl, rc);
        if (rc == 0)
...
static void make_random_str(char *str, int *len)
{
    ...
    sprintf(str, "%d-%d-%ld-%d", cdb2_hostid(), PID, tv.tv_usec, rand());
    *len = (int)strlen(str);
}
                                ...
                                char **query_hint)
{
    ...
    int len = (int)strlen(sqlstr);
    int len_id = (int)strlen(short_identifier);
    if (len_id > 128) {
...
{
    int num_hosts = (int)dbinfo_response->n_nodes;
    *num_valid_hosts = 0;
    ...
    if (hndl->in_trans && (hndl->snapshot_file ||
        ...
        if (hndl->debug_trace) {
            ...
            fprintf(stderr, "td %u %s sending 'begin' to %s line %d\n",
                    (uint32_t)SELF(), __func__, host, __LINE__);
        }
        ...
        if (rc != 0) {
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s line %d send_query rc=%d returning 1\n", 
                        (uint32_t)SELF(), host, __LINE__, rc);
            }
            return 1;
        }
        int len = 0;
        ...
        if (hndl->sb == NULL) {
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s line %d: sb is NULL, next_record "
                        ...
                        (uint32_t)SELF(), __func__, __LINE__, rc);
            }
            return 1;
        }
        ...
        while (item != NULL) { /* Send all but the last query. */
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s resending '%s' to %s line %d\n",
                        (uint32_t)SELF(), __func__, item->sql, host,
                        __LINE__);
            }
            sbuf2write((char *)&hdr, sizeof(hdr), hndl->sb);
            ...
            rc =
                cdb2_read_record(hndl, (char **)(&hndl->first_buf), &reclen, NULL);
            if (rc) {
                if (hndl->debug_trace) {
                    fprintf(stderr, "td %u %s line %d: Can't read response "
                                    "from the db node %s\n",
                            (uint32_t)SELF(), __func__, __LINE__, host);
                }
                sbuf2close(hndl->sb);
                hndl->sb = NULL;
                return 1;
            ...
            if (hndl->first_buf != NULL) {
                hndl->firstresponse =
                    cdb2__sqlresponse__unpack(NULL, reclen, hndl->first_buf);
            } else {
                if (hndl->debug_trace) {
                    fprintf(stderr, "td %u %s line %d: Can't read response "
                                    "from the db node %s\n",
        ...
        if (hndl->debug_trace) {
            fprintf(stderr, "td %u %s line %d in_trans=%d snapshot_file=%d query_no=%d\n", 
                    (uint32_t)SELF(), __func__, __LINE__, hndl->in_trans,
                                      ...
                                      int line)
{
    ...
    if (hndl->debug_trace) {
        fprintf(stderr, "td %u %s line %d setting in_trans to 0 from line %d\n",
                (uint32_t)SELF(), __func__, __LINE__, line);
    }
...
read_record:
    ...
    if (hndl->debug_trace) {
        ...
        if (hndl && hndl->connected_host >= 0)
            host = hndl->hosts[hndl->connected_host];
        if (hndl->debug_trace) {
            fprintf(stderr, "td %u reading response from %s line %d rc=%d type=%d\n", 
                    (uint32_t)SELF(), host, __LINE__, rc, type);
        }
    }
        ...
        if (hndl->is_hasql || commit_file) {
            if (commit_file) {
                if (hndl->debug_trace) {
                    fprintf(stderr,
                            ...
                            (uint32_t)SELF(), __LINE__, retries_done);
                    fprintf(stderr, "td %u %s line %d setting in_trans to 1\n",
                            (uint32_t)SELF(), __func__, __LINE__);
                }
                hndl->in_trans = 1;
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s line %d goto retry_queries rc=%d, err_val=%d\n", 
                        (uint32_t) SELF(), __func__, __LINE__, rc, err_val);
            }
            goto retry_queries;
        }
        ...
        if (!is_commit || hndl->snapshot_file) {
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s line %d goto retry_queries err_val=%d\n", 
                        (uint32_t) SELF(), __func__, __LINE__, err_val);
            }
            goto retry_queries;
        }
    ...
    if (log_calls) {
        if (ntypes == 0)
            fprintf(stderr, "%p> cdb2_run_statement(%p, \"%s\") = %d\n",
                    (void*)(intptr_t)SELF(), hndl, sql, rc);
        else {
            fprintf(stderr, "%p> cdb2_run_statement_typed(%p, \"%s\", [",
                    (void*)(intptr_t)SELF(), hndl, sql);
...
int cdb2_numcolumns(cdb2_hndl_tp *hndl)
{
    ...
    if (hndl->firstresponse == NULL)
        ...
        rc = (int)hndl->firstresponse->n_value;
    if (log_calls) {
...
const char *cdb2_column_name(cdb2_hndl_tp *hndl, int col)
{
    ...
    if (log_calls)
        fprintf(stderr, "%p> cdb2_column_name(%p, %d) = \"%s\"\n",
                (void*)(intptr_t)SELF(), hndl, col, ret == NULL ? "NULL" : ret);
    return ret;
}
...
const char *cdb2_errstr(cdb2_hndl_tp *hndl)
{
    ...
    if (log_calls)
        fprintf(stderr, "%p> cdb2_errstr(%p) = \"%s\"\n",
                (void*)(intptr_t)SELF(), hndl, ret ? ret : "NULL");
    return ret;
}
...
int cdb2_column_type(cdb2_hndl_tp *hndl, int col)
{
    ...
    if (log_calls) {
        fprintf(stderr, "%p> cdb2_column_type(%p, %d) = %s\n",
                (void*)(intptr_t)SELF(), hndl, col, cdb2_type_str(ret));
    }
    return ret;
}
...
int cdb2_column_size(cdb2_hndl_tp *hndl, int col)
{
    if (hndl->lastresponse == NULL)
        return -1;
    return (int)hndl->lastresponse->value[col]->value.len;
}
                    ...
                    const void *varaddr, int length)
{
    ...
    if (log_calls)
        fprintf(stderr, "%p> cdb2_bind_param(%p, \"%s\", %s, %p, %d) = 0\n",
                (void*)(intptr_t)SELF(), hndl, varname, cdb2_type_str(type),
                varaddr, length);
    return 0;
}
                    ...
                    const void *varaddr, int length)
{
    ...
    if (log_calls)
        fprintf(stderr, "%p> cdb2_bind_index(%p, %d, %s, %p, %d)\n",
                (void*)(intptr_t)SELF(), hndl, index, cdb2_type_str(type),
                varaddr, length);
    return rc;
}
                                ...
                                char *cluster, int *dbnum)
{
    ...
    *dbnum = 0;
    ...
    SOCKET fd = cdb2_socket_pool_get(newsql_typestr, comdb2db_num, NULL);
    if (fd == INVALID_SOCKET) {
        if (!allow_pmux_route) {
            fd = cdb2_tcpconnecth_to(host, port, 0, CDB2_CONNECT_TIMEOUT);
        } else {
    ...
    *num_hosts = 0;
    while (sqlresponse->response_type <= RESPONSE_TYPE__COLUMN_VALUES) {
        ...
        if (sqlresponse->response_type == RESPONSE_TYPE__COLUMN_VALUES &&
            ...
            if (*dbnum == 0) {
                *dbnum = (int)*((long long *)sqlresponse->value[1]->value.data);
            }
            (*num_hosts)++;
        }
    }
    cdb2__sqlresponse__free_unpacked(sqlresponse, NULL);
    free(p);
    cdb2_socket_pool_donate_ext(newsql_typestr, fd, 10,
                             ...
                             int *num_valid_sameroom_hosts)
{
    ...
    cdb2_socket_pool_donate_ext(newsql_typestr, fd, 10, dbnum, 5,
                                NULL, NULL);
    ...
    while (machine) {
        ...
        if (num_hosts < MAX_NODES) {
            if (strlen(hostname) >= sizeof(hndl->hosts[0]))
                fprintf(stderr, "Hostname \"%s\" is too long, max %zd\n",
...
static int set_up_ssl_params(cdb2_hndl_tp *hndl)
{
    /* In case that the application connects to multiple databases
       ...
       parameters to the handle and reset them. */
    char *sslenv;
...
MUTEX_T fend_ssl_init_lock = MUTEX_INITIALIZER;
int cdb2_init_ssl(int init_libssl, int init_libcrypto)
{
    int rc = 0;
    if (cdb2_called_ssl_init == 0 &&
        (rc = MUTEX_LOCK(&fend_ssl_init_lock)) == 0) {
        if (cdb2_called_ssl_init == 0) {
        ...
        if (rc == 0)
            rc = MUTEX_UNLOCK(&fend_ssl_init_lock);
        else
            MUTEX_UNLOCK(&fend_ssl_init_lock);
    }
    return rc;
}
...
static cdb2_ssl_sess_list *cdb2_get_ssl_sessions(cdb2_hndl_tp *hndl)
{
    ...
    int rc = MUTEX_LOCK(&cdb2_ssl_sess_lock);
    if (rc != 0)
        return NULL;
    ...
    *handle = hndl = calloc(1, sizeof(cdb2_hndl_tp));
    ...
    if (log_calls) {
        fprintf(stderr, "%p> cdb2_open(dbname: \"%s\", type: \"%s\", flags: "
                        "%x) = %d => %p\n",
                (void*)(intptr_t)SELF(), dbname, type, flags, rc, *handle);
    }
    return rc;
}
