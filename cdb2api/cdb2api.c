...
#define CDB2_CACHE_SSL_SESS_TTL_DEFAULT 0
static int cdb2_cache_ssl_sess_ttl = CDB2_CACHE_SSL_SESS_DEFAULT;
...
struct cdb2_ssl_sess_list {
    ...
    SSL_CTX *ctx;
    time_t tm; /* Time the certificate was cached */
    char dbname[64];
    char cluster[64];
    int ref;
    int n;
...
static int try_ssl(cdb2_hndl_tp *hndl, SBUF2 *sb, int indx)
{
    ...
    if (hndl->ctx == NULL) {
        rc = ssl_new_ctx(&hndl->ctx, hndl->c_sslmode, hndl->sslpath, &hndl->cert, &hndl->key, &hndl->ca, &hndl->crl,
                         hndl->num_hosts, NULL, hndl->min_tls_ver, hndl->errstr, sizeof(hndl->errstr));
        if (rc != 0) {
            hndl->sslerr = 1;
            return -1;
        }
    ...
    rc = sslio_connect(sb, hndl->ctx, hndl->c_sslmode, hndl->dbname, hndl->nid_dbname, ((p != NULL) ? p->sess : NULL));
...
static int process_ssl_set_command(cdb2_hndl_tp *hndl, const char *cmd)
{
    ...
    if (rc == 0) {
        ...
        /* Reset SSL context to swap in new certificates. */
        if (hndl->sess_list) {
...
static int cdb2_add_ssl_session(cdb2_hndl_tp *hndl)
{
    ...
    /* If the context has changed (most likely due to certificate TTL expiry),
       refresh the context. */
    if (sess_list != NULL && hndl->sess_list->ctx != hndl->ctx) {
        sess_list->ctx = hndl->ctx;
        sess_list->tm = time(NULL);
    }
    ...
    if (sess_list == NULL) {
        ...
        if (sess_list == NULL)
            return ENOMEM;
        sess_list->list = NULL;
        strncpy(sess_list->dbname, hndl->dbname,
                sizeof(hndl->dbname) - 1);
        sess_list->dbname[sizeof(hndl->dbname) - 1] = '\0';
        strncpy(sess_list->cluster, hndl->cluster,
